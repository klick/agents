...
class DiscoveryTxtService extends Component
{
    ...
    public function getLlmsFullTxtDocument(): ?array
    {
        $settings = $this->getSettings();
        if (!$settings->enableLlmsFullTxt) {
            return null;
        }

        $ttl = $this->normalizeTtl($settings->llmsTxtCacheTtl, 86400);
        return $this->getCachedDocument(self::CACHE_KEY_LLMS_FULL, $ttl, function() use ($settings): string {
            return $this->renderLlmsFullTxt($settings);
        });
    }
    ...
    public function invalidateAllCaches(): void
    {
        ...
        $cache->delete(self::CACHE_KEY_LLMS_FULL);
    ...
    public function prewarm(string $target = 'all'): array
    {
        ...
        if ($target === 'all' || $target === 'llms-full') {
            Craft::$app->getCache()->delete(self::CACHE_KEY_LLMS_FULL);
            $documents['llmsFull'] = $this->describeDocument($this->getLlmsFullTxtDocument());
        }
    ...
    public function getDiscoveryStatus(): array
    {
        ...
        $llmsFullTtl = $this->normalizeTtl($settings->llmsTxtCacheTtl, 86400);
        ...
        $llmsFullDocument = $settings->enableLlmsFullTxt ? $this->getLlmsFullTxtDocument() : null;
        ...
        return [
            ...
            'documents' => [
                ...
                'llmsFull' => $this->buildDocumentStatus('llms-full.txt', '/llms-full.txt', $settings->enableLlmsFullTxt, $llmsFullTtl, $llmsFullDocument),
    ...
    {
        $customBody = $this->resolveCustomBody($settings->llmsTxtBody);
        if ($customBody !== null) {
            return $customBody;
        }
    ...
    {
        $customBody = $this->resolveCustomBody($settings->commerceTxtBody);
        if ($customBody !== null) {
            return $customBody;
        }
    ...
    private function renderLlmsFullTxt(Settings $settings): string
    {
        $siteName = $this->getSiteName();
        $siteUrl = $this->getSiteUrl();
        $generatedAt = gmdate('Y-m-d\TH:i:s\Z');
        $summary = trim($settings->llmsSiteSummary);
        if ($summary === '') {
            $summary = 'Extended machine-readable discovery with integration and policy pointers.';
        }

        $lines = [
            '# ' . $siteName,
            '> llms-full.txt extended discovery surface generated by the Agents plugin.',
            '',
            'site: ' . $siteUrl,
            'last_generated: ' . $generatedAt,
            'summary: ' . $summary,
            '',
            '## Root Discovery Documents',
            '- llms.txt: ' . $this->toAbsoluteUrl('/llms.txt'),
            ...
            '- Capabilities: ' . $this->toAbsoluteUrl('/agents/v1/capabilities') . ' (auth required, scope: capabilities:read)',
            '- Auth Whoami: ' . $this->toAbsoluteUrl('/agents/v1/auth/whoami') . ' (auth required, scope: auth:read)',
            '- Products API: ' . $this->toAbsoluteUrl('/agents/v1/products?status=live&limit=100') . ' (auth required, scope: products:read)',
            '- Orders API: ' . $this->toAbsoluteUrl('/agents/v1/orders?status=all&limit=100') . ' (auth required, scope: orders:read)',
            '- Entries API: ' . $this->toAbsoluteUrl('/agents/v1/entries?status=live&limit=100') . ' (auth required, scope: entries:read)',
            '- Changes API: ' . $this->toAbsoluteUrl('/agents/v1/changes?types=products,orders,entries&limit=100') . ' (auth required, scope: changes:read)',
            '- Sections API: ' . $this->toAbsoluteUrl('/agents/v1/sections') . ' (auth required, scope: sections:read)',
        ];

        $customLinks = $this->normalizeLinks($settings->llmsLinks, 'Link');
        ...
        if (!empty($policyUrls)) {
            $lines[] = '';
            $lines[] = '## Commerce Policies';
            foreach ($policyUrls as $key => $value) {
                $lines[] = sprintf('- %s: %s', $key, $this->toAbsoluteUrl($value));
            }
        }
    ...
    private function resolveCustomBody(string $rawBody): ?string
    {
