...
expect_fixed "actionTargetSets" "$DASHBOARD_CONTROLLER" "Dashboard controller exposes dedicated Boundaries page"
...
expect_fixed "'label' => 'Boundaries'" "$PLUGIN_FILE" "Plugin exposes Boundaries in the Agents subnav"
expect_fixed "'agents/target-sets'" "$PLUGIN_FILE" "Plugin exposes the Boundaries route"
...
expect_fixed "Manage boundaries" "$CP_TEMPLATE" "Credentials CP template links to the dedicated Boundaries page"
...
expect_fixed "Agent Handoff" "$CP_TEMPLATE" "Credentials CP template labels the account handoff surface clearly"
...
expect_fixed "Agent Basics" "$CP_TEMPLATE" "Credentials CP template labels the safe default scope bundle clearly"
expect_fixed "Default on every new account:" "$CP_TEMPLATE" "Credentials CP template explains the default scope baseline"
expect_fixed "Added when creating a matching account from a Job:" "$CP_TEMPLATE" "Credentials CP template explains the Job-specific scope add-on"
expect_fixed ">Standard<" "$CP_TEMPLATE" "Credentials CP template exposes the standard scope section"
expect_fixed "credentialAdvancedScopes" "$CP_TEMPLATE" "Credentials CP template exposes the collapsed advanced scope section"
expect_fixed "agents-capability-pill-title" "$CP_TEMPLATE" "Credentials CP template renders human-readable scope labels"
expect_absent "agents-capability-pill-meta" "$CP_TEMPLATE" "Credentials CP template no longer exposes the raw scope identifier alongside the label"
...
expect_fixed "You set the boundary. Your agent works inside it." "$CP_TEMPLATE" "Credentials CP template frames managed accounts as governed agent boundaries"
expect_fixed "Broader than jobs" "$CP_TEMPLATE" "Credentials CP template flags accounts that are broader than attached jobs"
expect_fixed "job fit:" "$CP_TEMPLATE" "Credentials CP template exposes job fit details in account details"
...
expect_fixed "Start from a reusable account profile built for common agency jobs." "$CP_TEMPLATE" "Credentials CP template frames default templates as reusable agency jobs"
...
expect_fixed "<h1>Boundaries</h1>" "$TARGET_SETS_TEMPLATE" "Boundaries CP template is present"
expect_fixed "selectedSubnavItem = \"boundaries\"" "$TARGET_SETS_TEMPLATE" "Boundaries CP template selects the Boundaries subnav item"
expect_fixed "Reusable governed-write boundaries" "$TARGET_SETS_TEMPLATE" "Boundaries CP template explains the bounded write purpose"
